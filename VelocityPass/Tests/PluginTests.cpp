...
void testSampleAccurateMidi(Module& module) {
    ...
    for (const auto& channel : output) for (int i = 0; i < 32; ++i)
        close(channel[i], i < 8 ? 0 : i < 16 ? .5 : 1,
              "Sample-accurate MIDI mapping; off/zero messages ignored");
    EventList boundary;
    ...
    for (const auto& channel : next) for (int i = 0; i < 16; ++i)
        close(channel[i], i < 8 ? 1 : .5, "Back-to-back off/on section boundary");
}
...
void testParametersAndMidiPriority(Module& module) {
    ...
    for (const auto& channel : output) for (int i = 0; i < 24; ++i)
        close(channel[i], i < 8 ? .25 : i < 12 ? .5 : i < 16 ? .75 : 0,
...
void testEditorLifecycle(Module& module) {
    ...
    HWND parent = CreateWindowExW(0, L"STATIC", L"Velocity Pass integration test host",
...
void testLargeEventLists(Module& module) {
    ...
    for (const auto& channel : output) for (int i = 0; i < count; ++i)
        close(channel[i], (i % 127) / 126.0, "Large event batch preserves every step");
...
void testState(Module& module) {
    ...
    plugin.flush(&half, &enableBypass);
    MemoryStream state;
    ...
    allEqual(plugin.run(constant<float>(2, 16)), .5, "MIDI-held gain is restored");
    ...
    allEqual(plugin.run(constant<float>(2, 16)), .5, "Invalid state leaves the current gain intact");
}
} // namespace

int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "Usage: plugin_tests.exe <bundle/Contents/x86_64-win/VelocityPass.vst3>\n";
        return 2;
    }
