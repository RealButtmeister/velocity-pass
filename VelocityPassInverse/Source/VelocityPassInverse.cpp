...
namespace {
const FUID processorId(0x9379BEB5,0x0DEB46EB,0xBDCBA6E2,0xD023E53C);
const FUID controllerId(0x47ABBF5B,0xDCB241D9,0x8CBFC576,0x43F021D9);
constexpr ParamID kLevel=0, kBypass=1;
constexpr int32 never=std::numeric_limits<int32>::max();
const wchar_t* editorClass=L"EightBar_VelocityPassInverse_9379BEB5";
char moduleAddress;
...
double decodeVelocity(double velocity) {
    ...
    // Complement the original plugin's linear amplitude response exactly.
    return 1. - std::clamp((midi-1.)/126.,0.,1.);
}
...
public:
    ...
    tresult PLUGIN_API attached(void* parent,FIDString type) override {
        ...
        window=CreateWindowExW(0,editorClass,L"Velocity Pass Inverse",WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS,
...
private:
    ...
    void paint(HDC dc) {
        ...
        text(L"VELOCITY PASS INVERSE",28,22,17,RGB(232,240,245),FW_SEMIBOLD);
        text(L"Pass-through level",29,57,13,RGB(151,171,188));
        wchar_t amount[32];swprintf(amount,32,L"%.1f%%",level*100);
        text(amount,24,78,60,RGB(247,186,123),FW_SEMIBOLD);
        ...
        HBRUSH accent=CreateSolidBrush(RGB(247,186,123));FillRect(dc,&fill,accent);DeleteObject(accent);
        text(bypass?L"Bypassed in host":L"Higher velocity = quieter. Note-offs hold.",29,180,12,RGB(151,171,188));
...
DEF_CLASS2(INLINE_UID_FROM_FUID(processorId),PClassInfo::kManyInstances,kVstAudioEffectClass,
    "Velocity Pass Inverse",Vst::kDistributable,"Fx|Tools","1.0.0",kVstVersionString,Processor::create)
DEF_CLASS2(INLINE_UID_FROM_FUID(controllerId),PClassInfo::kManyInstances,kVstComponentControllerClass,
    "Velocity Pass Inverse Controller",0,"","1.0.0",kVstVersionString,Controller::create)
END_FACTORY
